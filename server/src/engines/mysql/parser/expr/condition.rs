...
use crate::engines::mysql::ast::{ComparisonOp, Condition, LogicalOp};
    ...
    while let Some(op_pair) = inner.next() {
        if op_pair.as_rule() == Rule::KW_OR {
            let right_pair = inner
                .next()
                .ok_or_else(|| SqlError::Parse("Missing condition after OR".to_string()))?;
...
pub fn parse_conjunction(pair: pest::iterators::Pair<Rule>) -> SqlResult<Condition> {
    ...
    let first = inner
        .next()
        .ok_or_else(|| SqlError::Parse("Empty conjunction".to_string()))?;
    ...
    while let Some(op_pair) = inner.next() {
        if op_pair.as_rule() == Rule::KW_AND {
            let right_pair = inner
                .next()
                .ok_or_else(|| SqlError::Parse("Missing condition after AND".to_string()))?;
...
pub fn parse_primary_condition(pair: pest::iterators::Pair<Rule>) -> SqlResult<Condition> {
    ...
    let first = inner
        .next()
        .ok_or_else(|| SqlError::Parse("Empty primary condition".to_string()))?;
    ...
    match first.as_rule() {
        ...
        Rule::KW_NOT => {
            let sub = inner
                .next()
                .ok_or_else(|| SqlError::Parse("Missing condition after NOT".to_string()))?;
        ...
        Rule::expression => {
            ...
            match next.as_rule() {
                Rule::comparison_op => {
                    ...
                    if op_str == "IN" {
                        let sub_pair = inner.next().ok_or_else(|| {
                            SqlError::Parse("Missing subquery after IN".to_string())
                        })?;
                ...
                Rule::KW_IS => {
                    ...
                    if tok.as_rule() == Rule::KW_NOT {
                        is_not = true;
                        tok = inner.next().ok_or_else(|| {
                            SqlError::Parse("Missing token after IS NOT".to_string())
                        })?;
                    }
                ...
                Rule::KW_IN => {
                    let sub_pair = inner
                        .next()
                        .ok_or_else(|| SqlError::Parse("Missing subquery after IN".to_string()))?;
                    let subquery = find_select_stmt(sub_pair)?;
...
pub fn find_select_stmt(
...
) -> SqlResult<crate::engines::mysql::ast::SelectStmt> {
    ...
    if pair.as_rule() == Rule::select_stmt {
        ...
        if let crate::engines::mysql::ast::SqlStmt::Select(s) = stmt {
            return Ok(s);
        }
    }
