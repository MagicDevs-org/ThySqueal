...
use crate::engines::mysql::ast::{SelectStmt, SetOperationClause, SetOperator, SqlStmt};
    ...
    if first.as_rule() == Rule::with_clause {
        ...
        for cte_pair in first.into_inner() {
            ...
            if cte_pair.as_rule() == Rule::cte_definition {
                ...
                ctes.push(crate::engines::mysql::ast::Cte { name, query });
            }
        }
        with_clause = Some(crate::engines::mysql::ast::WithClause { recursive, ctes });
        first = inner
            .next()
