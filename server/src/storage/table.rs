...
use crate::sql::eval::{Evaluator, evaluate_expression_joined, evaluate_condition_joined};
...
impl TableIndex {
    ...
    pub fn where_clause(&self) -> Option<Condition> {
        ...
        where_json.as_ref().and_then(|j| serde_json::from_value(j.clone()).ok())
    }
...
impl Table {
    ...
    pub fn create_index(
        ...
        let mut index = if use_hash {
            TableIndex::Hash { unique, expressions: expr_jsons, where_clause: where_json, data: HashMap::new() }
        } else {
            TableIndex::BTree { unique, expressions: expr_jsons, where_clause: where_json, data: BTreeMap::new() }
        };
        ...
        for row in &self.rows {
            ...
            if let Some(ref c) = cond {
                ...
                if !evaluate_condition_joined(evaluator, c, &context, &[], db_state).map_err(|e| StorageError::PersistenceError(format!("Index where clause evaluation error: {:?}", e)))? {
                    continue;
                }
            }
    ...
    fn extract_key(&self, evaluator: &dyn Evaluator, row: &Row, expressions: &[Expression], db_state: &DatabaseState) -> Result<Vec<Value>, StorageError> {
        self.extract_key_from_values(evaluator, &row.values, expressions, db_state)
    }

    pub fn extract_key_from_values(&self, evaluator: &dyn Evaluator, values: &[Value], expressions: &[Expression], db_state: &DatabaseState) -> Result<Vec<Value>, StorageError> {
    ...
    pub fn insert(&mut self, evaluator: &dyn Evaluator, values: Vec<Value>, db_state: &DatabaseState) -> Result<String, StorageError> {
        ...
        let row = Row { id: id.clone(), values: values.to_vec() };
        let table_ref: &Table = self;
        ...
        for index in self.indexes.values() {
            ...
            if let Some(cond) = index.where_clause() {
                let context = [(table_ref, &row)];
                if !evaluate_condition_joined(evaluator, &cond, &context, &[], db_state).map_err(|e| StorageError::PersistenceError(format!("Index where clause evaluation error: {:?}", e)))? {
                    continue;
                }
            }

            if index.is_unique() {
                let key = table_ref.extract_key_from_values(evaluator, &values, &index.expressions(), db_state)?;
    ...
    pub fn update(&mut self, evaluator: &dyn Evaluator, id: &str, values: Vec<Value>, db_state: &DatabaseState) -> Result<(), StorageError> {
        if values.len() != self.columns.len() {
            return Err(StorageError::InvalidType(format!(
                "Expected {} columns, got {}",
                self.columns.len(),
        ...
        if let Some(pos) = self.rows.iter().position(|r| r.id == id) {
            ...
            let old_row = Row { id: id.to_string(), values: old_values.clone() };
            let new_row = Row { id: id.to_string(), values: values.to_vec() };
            let table_ref: &Table = self;
            ...
            for index in self.indexes.values() {
                ...
                if index.is_unique() {
                    let new_key = table_ref.extract_key_from_values(evaluator, &values, &index.expressions(), db_state)?;
                    let old_key = table_ref.extract_key_from_values(evaluator, &old_values, &index.expressions(), db_state)?;
            ...
            for index in self.indexes.values() {
                ...
                let old_match = if let Some(ref c) = cond {
                    ...
                    evaluate_condition_joined(evaluator, c, &context, &[], db_state).map_err(|e| StorageError::PersistenceError(format!("Index where clause evaluation error: {:?}", e)))?
                } else {
                    true
                };
            ...
            for (index, (old_match, new_match, old_key, new_key)) in self.indexes.values_mut().zip(index_updates) {
    ...
    pub fn delete(&mut self, evaluator: &dyn Evaluator, id: &str, db_state: &DatabaseState) -> Result<(), StorageError> {
        if let Some(pos) = self.rows.iter().position(|r| r.id == id) {
