    ...
    pub(crate) fn get_result_column_names(
        ...
        for col in &stmt.columns {
            ...
            match &col.expr {
                ...
                squeal::ir::Expression::Column(name) => names.push(name.clone()),
                squeal::ir::Expression::FunctionCall(fc) => {
                    let name = format!("{:?}", fc.name).to_uppercase();
                    names.push(format!("{}(...)", name));
                }
                squeal::ir::Expression::ScalarFunc(sf) => {
