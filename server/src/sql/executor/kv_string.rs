...
impl Executor {
    pub async fn kv_set(&self, key: String, value: Value, tx_id: Option<&str>) -> SqlResult<()> {
        ...
        .map_err(super::super::error::SqlError::Storage)?;
        Ok(())
    }
    ...
    pub async fn kv_get(&self, key: &str, tx_id: Option<&str>) -> SqlResult<Option<Value>> {
        if let Some(id) = tx_id {
            let state = self.transactions.get(id).ok_or_else(|| {
                super::super::error::SqlError::Runtime("Transaction not found".to_string())
            })?;
    ...
    pub async fn kv_keys(&self, pattern: &str, tx_id: Option<&str>) -> SqlResult<Vec<String>> {
        ...
        let re = regex::Regex::new(&format!("^{}$", regex_pattern)).map_err(|e| {
            super::super::error::SqlError::Runtime(format!("Invalid pattern: {}", e))
        })?;
