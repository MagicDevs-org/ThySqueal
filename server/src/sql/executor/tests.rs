...
mod tests {
    ...
    async fn test_update() {
        let exec = Executor::new(crate::storage::Database::new());
        exec.execute("CREATE TABLE t (id INT, v TEXT)", None)
            .await
            .unwrap();
        exec.execute("INSERT INTO t (id, v) VALUES (1, 'old')", None)
            .await
            .unwrap();

        let r = exec
            .execute("UPDATE t SET v = 'new' WHERE id = 1", None)
            .await
            .unwrap();
        assert_eq!(r.rows_affected, 1);

        let r = exec
            .execute("SELECT v FROM t WHERE id = 1", None)
            .await
            .unwrap();
    ...
    async fn test_delete() {
        ...
        exec.execute("INSERT INTO t (id) VALUES (1)", None)
            .await
            .unwrap();
        exec.execute("INSERT INTO t (id) VALUES (2)", None)
            .await
            .unwrap();

        let r = exec
            .execute("DELETE FROM t WHERE id = 1", None)
            .await
            .unwrap();
        assert_eq!(r.rows_affected, 1);
    ...
    async fn test_order_by() {
        let exec = Executor::new(crate::storage::Database::new());
        exec.execute("CREATE TABLE t (id INT)", None).await.unwrap();
        exec.execute("INSERT INTO t (id) VALUES (3)", None)
            .await
            .unwrap();
        exec.execute("INSERT INTO t (id) VALUES (1)", None)
            .await
        ...
        let r = exec
            .execute("SELECT * FROM t ORDER BY id ASC", None)
            .await
            .unwrap();
        assert_eq!(r.rows[0][0].as_int(), Some(1));
        ...
        let r = exec
            .execute("SELECT * FROM t ORDER BY id DESC", None)
            .await
            .unwrap();
    ...
    async fn test_limit_offset() {
        ...
        let r = exec
            .execute("SELECT * FROM t ORDER BY id ASC LIMIT 3", None)
            .await
            .unwrap();
        ...
        let r = exec
            .execute("SELECT * FROM t ORDER BY id ASC LIMIT 3 OFFSET 2", None)
            .await
            .unwrap();
        assert_eq!(r.rows.len(), 3);
        assert_eq!(r.rows[0][0].as_int(), Some(3));
    ...
    async fn test_aggregations_and_aliases() {
        ...
        let r = exec
            .execute(
                "SELECT COUNT(*) AS total_count, SUM(amount) AS total_amount FROM sales",
                None,
            )
            .await
            .unwrap();
    ...
    async fn test_group_by_having() {
        let exec = Executor::new(crate::storage::Database::new());
        exec.execute(
            "CREATE TABLE orders (id INT, customer TEXT, amount FLOAT)",
            None,
        )
        .await
        .unwrap();
        ...
        .unwrap();
        exec.execute(
            "INSERT INTO orders (id, customer, amount) VALUES (2, 'bob', 200.0)",
            None,
        )
        .await
        .unwrap();
        exec.execute(
            "INSERT INTO orders (id, customer, amount) VALUES (3, 'alice', 50.0)",
            None,
        )
        .await
        .unwrap();
        exec.execute(
            "INSERT INTO orders (id, customer, amount) VALUES (4, 'charlie', 300.0)",
            None,
        )
        .await
        .unwrap();
    ...
    async fn test_inner_join() {
        ...
        exec.execute("CREATE TABLE users (id INT, name TEXT)", None)
            .await
            .unwrap();
        exec.execute("CREATE TABLE posts (id INT, user_id INT, title TEXT)", None)
            .await
            .unwrap();

        // Create index on user_id
        exec.execute("CREATE INDEX idx_user_id ON posts (user_id)", None)
            .await
            .unwrap();
        ...
        exec.execute(
            "INSERT INTO posts (id, user_id, title) VALUES (10, 1, 'p1')",
            None,
        )
        .await
        .unwrap();
        exec.execute(
            "INSERT INTO posts (id, user_id, title) VALUES (11, 1, 'p2')",
            None,
        )
        .await
        .unwrap();
        exec.execute(
            "INSERT INTO posts (id, user_id, title) VALUES (12, 2, 'p3')",
            None,
        )
        .await
        .unwrap();
    ...
    async fn test_left_join() {
        ...
        exec.execute("CREATE TABLE users (id INT, name TEXT)", None)
            .await
            .unwrap();
        exec.execute("CREATE TABLE posts (user_id INT, title TEXT)", None)
            .await
            .unwrap();
        ...
        exec.execute("INSERT INTO posts (user_id, title) VALUES (1, 'p1')", None)
            .await
            .unwrap();
    ...
    async fn test_distinct() {
        let exec = Executor::new(crate::storage::Database::new());
        exec.execute("CREATE TABLE t (name TEXT)", None)
            .await
            .unwrap();
        exec.execute("INSERT INTO t (name) VALUES ('alice')", None)
            .await
            .unwrap();
        exec.execute("INSERT INTO t (name) VALUES ('bob')", None)
            .await
            .unwrap();
        exec.execute("INSERT INTO t (name) VALUES ('alice')", None)
            .await
        ...
        let r = exec
            .execute("SELECT DISTINCT name FROM t", None)
            .await
            .unwrap();
        assert_eq!(r.rows.len(), 2);
    }
    ...
    async fn test_explain() {
        ...
        exec.execute("CREATE TABLE users (id INT, name TEXT)", None)
            .await
            .unwrap();
        exec.execute("CREATE INDEX idx_id ON users (id)", None)
            .await
            .unwrap();
        ...
        let r = exec
            .execute("EXPLAIN SELECT * FROM users WHERE name = 'alice'", None)
            .await
            .unwrap();
    ...
    async fn test_hash_index() {
        ...
        exec.execute("CREATE TABLE users (id INT, name TEXT)", None)
            .await
            .unwrap();
        exec.execute("CREATE INDEX idx_id_hash ON users (id) USING HASH", None)
            .await
            .unwrap();
    ...
    async fn test_unique_index() {
        let exec = Executor::new(crate::storage::Database::new());
        exec.execute("CREATE TABLE users (id INT, email TEXT)", None)
            .await
            .unwrap();
        exec.execute("CREATE UNIQUE INDEX idx_email ON users (email)", None)
            .await
            .unwrap();
    ...
    async fn test_composite_index() {
        ...
        .await
        .unwrap();
        exec.execute("SELECT * FROM users WHERE last_name = 'Smith'", None)
            .await
            .unwrap();
    }
    ...
    async fn test_json_path_index() {
        let exec = Executor::new(crate::storage::Database::new());
        exec.execute("CREATE TABLE events (id INT, data JSON)", None)
            .await
            .unwrap();
        // Index on a nested JSON field
        exec.execute("CREATE INDEX idx_event_id ON events (data.id)", None)
            .await
            .unwrap();

        exec.execute(
            "INSERT INTO events (id, data) VALUES (1, '{\"id\": 100, \"type\": \"signup\"}')",
            None,
        )
        .await
        .unwrap();
        exec.execute(
            "INSERT INTO events (id, data) VALUES (2, '{\"id\": 101, \"type\": \"login\"}')",
            None,
        )
        .await
        .unwrap();
        ...
        let r = exec
            .execute("EXPLAIN SELECT * FROM events WHERE data.id = 100", None)
            .await
            .unwrap();
        assert!(
            r.rows[0][1]
    ...
    async fn test_functional_index() {
        ...
        exec.execute("CREATE INDEX idx_lower_email ON users (LOWER(email))", None)
            .await
            .unwrap();
    ...
    async fn test_partial_index() {
        let exec = Executor::new(crate::storage::Database::new());
        exec.execute("CREATE TABLE orders (id INT, status TEXT)", None)
            .await
            .unwrap();
        // Index only pending orders, enforce uniqueness on id for them
        exec.execute(
            "CREATE UNIQUE INDEX idx_pending_id ON orders (id) WHERE status = 'pending'",
            None,
        )
        .await
        .unwrap();
