    ...
    loop {
        ...
        match cmd_name.as_str() {
            ...
            "XADD" => {
                ...
                let start_idx = if cmd_array.len() > 2 && matches!(&cmd_array[2], RespValue::BulkString(Some(b)) if !b.starts_with(b"*")) { 3 } else { 2 };
                
                let mut fields = HashMap::new();
                let mut i = start_idx;
                while i + 1 < cmd_array.len() {
                    let field = extract_bulk_string(&cmd_array[i])?;
                    let value = extract_value(&cmd_array[i + 1]).unwrap_or(Value::Text("".to_string()));
            ...
            "XRANGE" => {
                ...
                let count = if cmd_array.len() > 4 && matches!(&cmd_array[4], RespValue::BulkString(Some(b)) if b == b"COUNT") {
                    Some(extract_integer(&cmd_array[5])? as usize)
                } else {
                    None
                };
                
                let results = executor.kv_stream_range(&key, &start, &stop, count, None).await?;
                ...
                for (id, fields) in results {
                    ...
                    for (field, value) in fields {
                        ...
                        field_values.push(RespValue::BulkString(Some(format!("{:?}", value).into_bytes())));
                    }
